...
b2Body* PhysicsManager::CreateBody(ISceneObject *obj, b2BodyType bodyType, u32 shapeForm, b2Vec2 *customSize)
{
	if (!obj)
		return NULL;
	...
	if(shapeForm == ShapeForm::BOX)
	{
		b2PolygonShape shape;

		if(customSize)
			shape.SetAsBox(customSize->x * 0.5f * PIX2M, customSize->y * 0.5f * PIX2M);
		else
			shape.SetAsBox(obj->GetWidth() * 0.5f * PIX2M, obj->GetHeight() * 0.5f * PIX2M);
